...
def generate(input_prefix, top_n, env, encoder, decoder, device, beam_size=10, include_logp=True):
    input_infix = prefix_to_infix(input_prefix)
    input_sympy = infix_to_sympy(input_infix)
    input_sympy = sp.simplify(input_sympy)
    input_sympy = sp.expand(input_sympy)
    res = []
    if input_sympy.func == sp.core.add.Add:
        de = len(input_sympy.args)
        for i in range(de):
            decom_prefix = sympy_to_prefix(input_sympy.args[i])
            x1_prefix = env.clean_prefix(['sub', 'derivative', 'f', 'x', 'x'] + decom_prefix)
            
            x1 = torch.LongTensor(
                [env.eos_index] +
                [env.word2id[w] for w in x1_prefix_c] +
                [env.eos_index],
            ).view(-1, 1).to(device)
            ...
            with torch.no_grad():      
                seq,seq_len = decoder.generate(encoded, len1, max_len=200)
                
                ids_list = seq[1:len(seq)-1].tolist()
                ids = []
                for list_number in ids_list:
                    number = list_number[0]
                    ids.append(number)
                hyp_toks = [env.id2word[wid] for wid in ids]
            hyp_infix_decom = env.prefix_to_infix(hyp_toks)
            res.append(hyp_infix_decom)
        
    res_infix = '+'.join(res)
    res_sympy = infix_to_sympy(res_infix)
    res_sympy = sp.simplify(res_sympy)
    hyp_res = sympy_to_prefix(res_sympy)       
    
    hyps = []
    
    try:
        hyp_infix = env.prefix_to_infix(hyp_res)  # convert to infix
    except:
