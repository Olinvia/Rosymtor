...
def int_to_coefficients(expr):
    i = 0
    a_index = -1
    a = OrderedDict()
    while i < len(expr):
        pos = i
        if expr[i] == 'INT+' or expr[i] == 'INT-':
            pos = i + 1
            a_index = a_index + 1
            res = ""
            while pos < len(expr) and expr[pos].isdigit():
                ...
                pos = pos + 1
            res = int(res)
            a['a' + str(a_index)] = (1 if expr[i] == 'INT+' else -1) * res
            expr = expr[:i] + ['a' + str(a_index)] + expr[pos:]
            i = pos
        else:
            i = i + 1
...
def coefficients_to_int(expr, a):
    i = 0
    pattern = "a\d+"
    while i < len(expr):
        if re.match(pattern, expr[i]):
            # print(a[expr[i]])
            if a[expr[i]] >= 0:
                offset = len(str(a[expr[i]]))
                pos = i + offset + 1
                expr = expr[:i] + ['INT+'] + list(str(a[expr[i]])) + expr[i+1:]
                i = pos
            else:
                offset = len(str(abs(a[expr[i]])))
                pos = i + offset + 1
                expr = expr[:i] + ['INT-'] + list(str(abs(a[expr[i]]))) + expr[i+1:]
                i = pos
        else:
            i = i + 1
    return expr    
...
def generate(input_prefix, top_n, env, encoder, decoder, device, beam_size=10, include_logp=True):
    ...
    x1_prefix_c, a = int_to_coefficients(x1_prefix)
    # print(x1_prefix_c)
    ...
    hyp_toks_c = coefficients_to_int(hyp_toks, a)
    #print(hyp_toks_c)
    
    hyps = []
    
    try:
        hyp_infix = env.prefix_to_infix(hyp_toks_c)  # convert to infix
    except:
